...
## Overview
This blog post introduces a new perspective to understand the **Random Feature Attention (RFA)** mechanism. In our [ICML 2022](https://proceedings.mlr.press/v162/zheng22b.html) paper <d-cite key="lara"></d-cite>, we show that **1)** the conventional sofmtax attention can be equivalently rewritten as an expectation over simple RFAs, and that **2)** RFA is in fact a **self-normalized importance sampler** to estimate conventional sofmtax attention. This new perspective not only grounds the heuristic RFA approximation, but also sheds light on how to further generalize and improve RFAs.

## Attention
The attention mechanism <d-cite key="bahdanau2014neural,vaswani2017attention"></d-cite> is a ubiquitous building block in modern deep learning systems and has brought great success across various domains, including natural language processing (NLP), computer vision (CV), bioinformatics, reinforcement learning, etc. 
Attention mechanisms take three different kinds of inputs: the set of query vectors $\mbQ \in \R^{N \times D}$, key vectors $\mbK \in \R^{M \times D}$ and value vectors $\mbV \in \R^{M \times D}$.<d-footnote> In this work we focus on self-attention, where $M=N$ and all of queries, keys and values are obtained by projecting tokens of the same input sequence. </d-footnote>
<div class="row mt-3">
...
\end{equation}
Intuitively, softmax attention first computes the normalized similarity between the query and each key, which is then used to weight value vectors. It is effective in capturing long-term dependencies across sequence elements and producing contextualized representations; however, they often suffer from **quadratic** time and memory complexity due to the explicit comparison between all query-key pairs. 
<div class="row mt-3">
...
To reduce the computational complexity of softmax attention, recently researchers proposed to use Random Features (RF; <d-cite key="random-features"></d-cite>) to linearize softmax atttention (**RFA** <d-cite key="peng2021rfa"></d-cite>; also called **Performer** <d-cite key="choromanski2021rethinking"></d-cite>). In particular, they make use of the following identiy to rewrite the **exponential kernel** as an expectation,
\begin{equation}
...
<div class="row mt-3" style="border: 1px solid #000;background-color: #ebebeb; border-radius: 10px; padding-top: 30px">
    ...
    This is due to the fun fact that the sample average can be written as $\mbphi(\mbx,\mbw)^\top \mbphi(\mby,\mbw)$, where $\mbphi(\mbx,\mbw) \coloneqq  1/\sqrt{S}[\xi(\mbx,\omega_1), \dots, \xi(\mbx, \omega_S)]^\top \in \R^{lS}$. The $\mbphi(\cdot,\cdot)$ can be considered as a feature function transforming the input vector to a new vector representation; as a result, $\mbphi(\cdot,\cdot)$ are conventionally referred to as random features <d-cite key="random-features"></d-cite>.</p>
...
RFA suffers from significant performance degradation across various tasks, as observed in many previous studies. Although computationally efficient, the reduced modeling capacity greatly limits its practical usage. Researchers investigate this problem and try to improve the performance from different perspectives, such as developing better random feature types <d-cite key="likhosherstov2022chefs, choromanski2022hybrid, chowdhury2021learning"></d-cite>, connecting it with fast weight programmers <d-cite key="pmlr-v139-schlag21a, irie2021going"></d-cite>, incorporating relative information <d-cite key="spe, luo2021stable, ripple, zhen2022cosformer"></d-cite>, etc.

We explore an orthogonal axis by re-examining the estimation bias of RFA. Our key observation is that RFA is a heuristic approximation to the whole softmax attention. Although the estimation of individual exponential kernels is unbiased, such estimation of a ratio of exponential kernels is not unbiased anymore. This is due to the non-linearity of ratios,
<div class="row mt-3">
...
<div class="row mt-3">
    <p>
    Our work aims to answer the following research question:<br>
...
<p>
We answer this question in the affirmative and prove that softmax attention can be rewritten as an expectation over simple RFAs,
$$
\begin{equation}
\mathsf{SoftmaxAttn}(\mbq_n, \mbK,\mbV) = \mathbb{E}_{p_n(\omega)}\left[f_n(\omega)\right],\label{eqn:softmax_as_expectation}
\end{equation}
$$
where
$$
...
<ul>
  ...
  <li>$f_n(\omega)$ is an RFA-like aggregation function over value vectors, which computes the linearized similarity between queries and keys via randomized mappings (or equivalently, one-dimensional random features).</li>
</ul></p>

<p>
This implies we can basically construct a Monte Carlo estimator to approximate softmax attention in an unbiased way. By drawing $\omega_n \sim p_n(\omega)$, we obtain
$$
\begin{align}
...
$$
\begin{equation}
    \mathbb{E}_{p_n(\omega)}\left[f_n(\omega)\right] \approx \frac{\sum_{s=1}^S\frac{p_n(\omega_s)}{q(\omega_s)}f(\omega_s)}{\sum_{s=1}^S\frac{p_n(\omega_s)}{q(\omega_s)}}. \label{eqn:snis}
\end{equation}
$$
The name <b>self-normalized</b> comes from the fact that the importance weights $p(\omega)/q(\omega)$ are normalized and sum to 1.
</p>
...
### RFA as SNIS
<p> Our next finding is that the formulation of RFA can be exactly derived from SNIS. Suppose $p_n(\omega)$ and $f_n(\omega)$ are given in \eqref{eqn:ra-density} and \eqref{eqn:ra-function} respectively, and $q(\omega) = \mathcal{N}(\omega;0,\mathbf{I})$. Then we have
$$
\begin{align}
...
\end{align}
$$
This formulation provides a new understanding of RFA: it is just a specific <b>SNIS</b> estimator for softmax attention, whose proposal distribution $q(\omega)$ is chosen to be standard Gaussian. This means we can generalize and extend RFA by investigating its importance sampling nature further! More importantly, it reveals one of the possible reasons why RFA does not work well in practice: <b>The plain standard Gaussian proposal in RFA is far away from the true Gaussian mixture (as in RA), which might lead to a large approximation gap</b>.</p>

## LARA: a generalized RFA beyond simple standard Gaussian proposals

From our analysis, it seems promising to use the Randomized Attention (RA) to approximate softmax attention, as it is not only **unbiased** in theory but also directly draws from the true distribution $p_n(\omega)$. This makes the mechanism **query-specific**, since the sampling distribution depends on the query vector \eqref{eqn:ra-density}. As a result, RA can specialize to each query and process the whole sequence at a finer-grained level. 

However, it suffers from **quadratic** computational costs. Since the sampling distribution is distinct for each query, we have to draw at least $\mathcal{O}(N)$ samples for all queries; at the same time, we needs to sum over all $M$ key-value pairs at each sampled $\omega_n$ (see \eqref{eqn:ra}), which leads to $\mathcal{O}(NM)$ complexity overall, as is the case for softmax attention. **This is in contrast to RFA, which uses SNIS to avoid computing or sampling from the true complicated distribution. Drawing from a simpler proposal distribution allows samples to be shared across queries and still resulting in a $\mathcal{O}(M)$ complexity.**
...
We attempt to get the best of both worlds, combining both the expressiveness of RA and the efficiency of RFA. Our key insight here is that the SNIS formulation allows us to improve RFA by designing **better** proposal distributions, while still admitting linear-time computation. In particular,
- for **efficiency**, we 
...
<p>
The resulting algorithm, which runs in linear complexity, is named <b>LARA</b> and takes the following form
$$
\begin{align}    
...
\end{align}
$$
Here $q_1(\omega), \dots, q_C(\omega)$ are $C$ different proposals, $\{\alpha_{nc}(\cdot)\}_{c=1}^C$ is a set of scalar weights that controls the contribution of different proposals. To maintain the consistency of the estimation, we need $\sum_{c=1}^C \alpha_{nc}(\omega) = 1$.
</p>
...
LARA provides a generalized estimation framework that includes both RA and RFA as its special cases. To be specific,
<ul>
...
We further verify the improved performance of LARA by conducting experiments across a wide range of data modalities, including images, videos, natural language texts and a long-sequence benchmark. We observe that
...
- Currently, LARA is still a biased (but consistent) estimator for softmax attention. It would be interesting to adopt advanced sampling strategies (e.g., Markov chain Monte Carlo (MCMC), etc.) to derive lower-bias lower-variance behaved estimators. 
- The proposal distribution should be carefully designed when dealing with auto-regressive modeling. It is highly non-trivial to apply the causal masking mechanism to LARA as in conventional softmax attention. Adapting the generalized SNIS estimation to support auto-regressive models is also an interesting research direction.  
...
- The resulting LARA is robust to random weight schedule: just redrawing weight each iteration works well. For deterministic evaluation, replace weight to their mean (just as dropout) works well.
